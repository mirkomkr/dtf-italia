...
const FileUploader = dynamic(
  () => import("../../components/configurator/shared/FileUploader"),
  {
    loading: () => (
      <div className="text-center p-8 text-gray-500">
        Caricamento Uploader...
      </div>
    ),
    ssr: false,
  },
    ...
    return (
      <div className="min-h-[50vh] flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-gray-600 max-w-md">
          Non siamo riusciti a identificare il tuo ordine. Assicurati di aver
          utilizzato il link corretto ricevuto via email.
        </p>
      </div>
    );
  }
  ...
  if (isUploadComplete) {
    return (
      <div className="min-h-[50vh] flex flex-col items-center justify-center p-6 text-center animate-in fade-in zoom-in duration-500">
        ...
        <a
          href="/"
          className="px-8 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition-colors shadow-xl shadow-indigo-200"
        >
          Torna alla Home
        </a>
      </div>
    ...
    <div className="max-w-4xl mx-auto px-6 py-12 md:py-20">
      <div className="text-center mb-12">
        <h1 className="text-4xl font-extrabold text-gray-900 mb-4 tracking-tight">
          Carica i file per l'ordine{" "}
          <span className="text-indigo-600">#{orderId}</span>
        </h1>
      ...
      <div className="bg-white rounded-3xl shadow-2xl shadow-indigo-100/50 p-6 md:p-10 border border-slate-100">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 mb-8 flex gap-4 items-start md:items-center">
          ...
          <div>
            ...
            <p className="text-amber-800 text-sm md:text-sm mt-1">
              Assicurati che i file siano pronti per la stampa (300 DPI, Sfondo
              Trasparente, CMYK).
            </p>
          </div>
        </div>
        ...
        <FileUploader
          ...
          onUploadComplete={async (uploadedFiles) => {
            const fileKey =
              uploadedFiles?.[0]?.s3Key || uploadedFiles?.[0]?.key;

            if (fileKey) {
              try {
                // Notifica WooCommerce con la chiave S3 reale
                // Salviamo su tutte le chiavi note per garantire compatibilità
                // con i bottoni download PHP (cercano _s3_file_key_front/_s3_file_key_center)
                await fetch("/api/order/update-metadata", {
                  ...
                  body: JSON.stringify({
                    orderId: orderId,
                    metaData: {
                      _s3_file_key: fileKey, // legacy
                      _s3_file_key_front: fileKey, // alias per serigrafia cap/shopper
                      _s3_file_key_center: fileKey, // posizione fronte cappello
...
export default function UploadPage() {
  return (
    <Suspense
      fallback={<div className="text-center py-20">Caricamento...</div>}
    >
      <UploadPageContent />
    </Suspense>
  );
