...
/**
 * ProductCard - Reusable product card component for catalogs
 * 
 * @param {Object} props
 * @param {Object} props.product - Product data from WooCommerce
 * @param {boolean} props.priority - Image loading priority (LCP optimization)
 * @param {string} props.basePath - Base path for product links (e.g., '/catalog/serigrafia')
 */
const ProductCard = memo(function ProductCard({ 
  product, 
  priority = false,
  basePath = '/stampa-serigrafica' // Default for backward compatibility
}) {
  ...
  const imageAlt =
    image?.alt ||
    `${product.name} personalizzato - DTF Italia`;
